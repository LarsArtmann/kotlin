...
class KtTypeRendererTestX : KotlinLightCodeInsightFixtureTestCase() {
    ...
    private fun doTestByTypeText(
        ...
        val renderedType = executeOnPooledThreadInReadAction {
            analyse(fakeKtFile) {
                ...
                ktType as KtFirType
                val session = (property.getSymbol() as KtFirSymbol<*>).firRef.withFir { it.session }
                val options = FirConeTypeRendererOptions().also { it.shortQualifiedNames = rendererOptions.renderFqNames }
                val x = ConeTypeRenderer(session, options).renderType(ktType.coneType)
                x
